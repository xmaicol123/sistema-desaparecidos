...
<!--HACIENDO METODO POST EN MISMA PAGINA PRUEBA-->
		<?php
			if ($_GET['setCod']) {
		?>
		<form method="POST" action="regEncontradoAdmin.php" enctype="multipart/form-data">
		<div class="container">
			<div class="row">
				<h2 class="text-center">Registro De Persona Encontrada</h2>
				...
				<div class="col-md-6">
					<div class="form-group">
						<center>
						<a href="regEncontradoAdmin.php?setCambiarImagen=<?php echo $numero = 1; ?>&&setCodEncontrado=<?php echo $_GET['setCod'] ?>&&setNombre=<?php echo $_GET['setNombre'] ?>&&setSexo=<?php echo $_GET['setSexo'] ?>&&setEdad=<?php echo $_GET['setEdad'] ?>&&setEstatura=<?php echo $_GET['setEstatura'] ?>&&setTez=<?php echo $_GET['setTez'] ?>&&setOjos=<?php echo $_GET['setOjos'] ?>&&setCabello=<?php echo $_GET['setCabello'] ?>&&setSeñasParticulares=<?php echo $_GET['setSeñasParticulares'] ?>&&setVestimenta=<?php echo $_GET['setVestimenta'] ?>&&setCircunstancia=<?php echo $_GET['setCircunstancia'] ?>&&setEstado=<?php echo $_GET['setEstado'] ?>&&setFechaRegistro=<?php echo $_GET['setFechaRegistro'] ?>"><h3 class="bg-primary lg" style="padding: 21px;">¿Usted Desea Cambiar De Imagen?</h3></a>
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						...
						<select class="form-control" name="txt_desaparicion">
							<option <?php if ($_GET['setEstado']=="Busqueda") {echo "selected";} ?> value="Busqueda">Busqueda</option>
							<option <?php if ($_GET['setEstado']=="Localizada") {echo "selected";} ?> value="Localizada">Localizada</option>
						</select>
					</div>
		...
		<div class="container">
			<div class="row">
				<h2 class="text-center">Registro De Persona Encontradas</h2>
				<input type="text" name="txt_codEncontrado" value="<?php echo $_GET['setCodEncontrado'] ?>" hidden>
				<input type="text" name="txt_fecha_registro" value="<?php echo $_GET['setFechaRegistro'] ?>" hidden>
				<div class="col-md-6">
					<div class="form-group">
						<center>
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						<label for="">Circunstancia</label>
						<textarea name="txt_circunstancia" class="form-control"></textarea>
					</div>
					<div class="form-group text-center" id="buttonPadding">
		...
		<!--CODIGO PHP :D-->
		<?php 
		include('../php/PhpregEncontrado.php');
			function guardar()
			{
				if ($_FILES['imagen']) 
				{
					...
					if ($tamagno_imagen<=2000000) 
					{
						if ($tipo_imagen=="image/jpeg" OR $tipo_imagen=="image/jpg" OR $tipo_imagen=="image/png" OR $tipo_imagen=="image/gif") 
						{
							...
							$obj->setNombreEncontrado($_POST['txt_nombre']);
			...
			function modificar()
			{
				if ($_POST['txt_cod']) 
				{
					if($_POST['txt_cod']) {
						$obj = new Encontrado();
						$obj->setCodEncontrado($_POST['txt_cod']);
						...
						$obj->setEstadoPersona($_POST['txt_desaparicion']);
						if ($obj->modificar()) {
							echo '<script>
				...
				if ($_FILES['imagen']) 
				{
					...
					if ($tamagno_imagen<=2000000) 
					{
						if ($tipo_imagen=="image/jpeg" OR $tipo_imagen=="image/jpg" OR $tipo_imagen=="image/png" OR $tipo_imagen=="image/gif") 
						{
							...
							/*Fecha Actual*/
							$obj = new Encontrado();
							$obj->setCodEncontrado($_POST['txt_codEncontrado']);
							$obj->setImagen($nombre_imagen);
							$obj->setNombreEncontrado($_POST['txt_nombre']);
							$obj->setSexo($_POST['txt_sexo']);
							$obj->setEdad($_POST['txt_edad']);
