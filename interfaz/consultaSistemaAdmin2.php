		...
		<div class="contenido"><!-- /container -->
			...
			<header class="codrops-header">
				...
				<nav class="codrops-demos">
					<a href="consultaSistemaAdmin.php">Buscamos A....</a>
					<a class="current-demo" href="consultaSistemaAdmin2.php">Encontramos A....</a>
				</nav>
			</header>
			</div>
		</div><!-- /container -->
			...
			$obj = new Encontrado();
			$reg = $obj->mostrarPersonaBuscadaIndex();
			...
			<div class="container" style="padding: 10px; height: auto;">
				...
				<div class="1">
					<div class="col-md-12" >
							<h2 class="text-center text-capitalize"><?php echo $row->nombre_encontrado ?></h2>
							...
							<div class="col-md-2 text-center" >
									<a href="regEncontradoAdmin.php?setCod=<?php echo $row->cod_encontrado ?>&&setNombre=<?php echo $row->nombre_encontrado ?>&&setSexo=<?php echo $row->sexo ?>&&setEdad=<?php echo $row->edad ?>&&setEstatura=<?php echo $row->estatura ?>&&setTez=<?php echo $row->tez ?>&&setOjos=<?php echo $row->ojos ?>&&setCabello=<?php echo $row->cabello ?>&&setSeñasParticulares=<?php echo $row->señas_particulares ?>&&setVestimenta=<?php echo $row->vestimenta ?>&&setCircunstancia=<?php echo $row->circunstancia ?>&&setImagen=<?php echo $row->imagen ?>&&setEstado=<?php echo $row->estado_persona ?>&&setFechaRegistro=<?php echo $row->fecha_registro ?>"><button class="btn btn-primary" type="button" style="width: 100%; margin: 10px;">Editar</button></a>
									<a href="reportePersonaEncontradoAdmin.php?setCodEncontrado=<?php echo $row->cod_encontrado ?>"><button class="btn btn-primary " type="button" style="width: 100%; margin: 10px;">Mensaje</button></a>
							</div>
					</div>
				</div>
				</div>
